...
//
// out_mode values: 0x0=high-z pullup, 0x5=normal push-pull, 0xF=input disabled
...
// MAX32630FTHR: Red LED = P2.4, active low (mbed LED1)
const LED_PORT: u32 = 2;
const LED_PIN:  u32 = 4;
const NORMAL_DRIVE: u32 = 0x5;
...
fn main() -> ! {
    unsafe {
        // Set P2.4 to normal push-pull output
        let mode = out_mode(LED_PORT).read_volatile();
        out_mode(LED_PORT).write_volatile(
            (mode & !(0xF << (LED_PIN * 4))) | (NORMAL_DRIVE << (LED_PIN * 4)),
        );
